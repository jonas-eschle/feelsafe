...
| 74e | `allowDisarmAsDistress=true`: disarm trigger fires during active distress chain (G-014) | Mode with `isDistressMode=true`, `allowDisarmAsDistress=true`, and a `GpsArrivalDisarmTrigger` configured; session is currently running the distress chain (entered via `replaceWithDistressChain`) | Synthesize GPS update inside the trigger radius | `ChainEvent.userDisarmed` emitted, engine transitions to `EngineEnded` with `EndReason.disarm`; `SessionLog.endReason == disarm` |
| 74f | `allowDisarmAsDistress=false`: disarm trigger ignored during active distress chain (G-014) | Mode with `isDistressMode=true`, `allowDisarmAsDistress=false`, and a `GpsArrivalDisarmTrigger` configured; session is currently running the distress chain | Synthesize GPS update inside the trigger radius | No `userDisarmed` event emitted; engine continues running the distress chain to exhaustion; trigger evaluation log records "ignored: allowDisarmAsDistress=false"; final `SessionLog.endReason == chainExhausted` |
| 74g | `allowDisarmAsDistress=true` with `TimerDisarmTrigger` (G-014) | Mode with `isDistressMode=true`, `allowDisarmAsDistress=true`, `TimerDisarmTrigger(durationSeconds: 300)`; session running the distress chain | Advance fakeAsync clock by 300 s | `ChainEvent.userDisarmed` emitted at 300 s, `EngineEnded` with `EndReason.disarm` |
| 74h | `allowDisarmAsDistress=false` with `TimerDisarmTrigger` (G-014) | Same as 74g but `allowDisarmAsDistress=false` | Advance fakeAsync clock by 300 s | Timer expires silently; no disarm event; distress chain continues |
| 74i | Regular mode (not running as distress) ignores `allowDisarmAsDistress`; disarm triggers fire normally regardless of the flag (G-014) | Mode with `isDistressMode=false`, `allowDisarmAsDistress=false`, GPS arrival configured; session running the main chain | Synthesize GPS update inside the trigger radius | `userDisarmed` emitted; flag has no effect outside distress mode |
