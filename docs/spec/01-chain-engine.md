...
- The "a session was running" marker is **not a separate JSON file** — it is the **in-progress `SessionLog` row** itself. `SessionController.startSession` upserts a `SessionLog` (carrying `modeId`, the snapshotted `modeName`, and `startedAt`, with **no `endedAt`**) at session start; the clean-end finaliser (`endSession` → `_finaliseLog`) deletes that row on a normal end. So a left-over orphan row (`endedAt == null`) is precisely the "process died mid-session" signal — one on-disk artifact, not two.
- On next app launch, `SessionController.build` detects the newest orphan `SessionLog`, captures its `modeId`/`modeName`/`startedAt`, and **deletes all orphan rows immediately** (so the prompt fires exactly once). While the resulting `SessionState.priorInterrupted` flag is set, the app shows an informational "Session-Interrupted" prompt (spec 04 §Extra-13) — surfaced by `HomeScreen` (the `/` landing route), since cold launch lands on home, never `/session`. The user acknowledges (→ home). Optionally, when the original `modeId` still resolves to a live mode, the user can start a fresh session of the same mode from the prompt (a brand-new session, not a resume — the engine starts at step 0 with a fresh `SessionLog`); if the mode was deleted, only the snapshotted `modeName` is shown and the [Start same mode] button is hidden.
- **Duress-PIN carve-out:** a distress/Duress cold-start session (the App-lock launch gate firing the resolved distress mode on a Duress-PIN entry) is started with `writeInterruptMarker: false`, so it **intentionally writes no in-progress marker row** and is **exempt from interrupted-prompt detection**. This is a security requirement, not a bug: a "Mode: Default Distress" prompt on the next launch would reveal a covert Duress run to an attacker who force-stopped the app, defeating the Duress PIN. Do not "fix" this by always writing the marker.
- **Rationale:** Lessons-learned §5.2 (memory + design intent): "App-death = session is gone, period." Reconstructing chain state on cold launch opens forensic-leak surfaces and breaks the "session = single contiguous in-memory run" engine invariant. The marker exists only to DETECT the interruption, inform the user, and optionally seed a fresh restart; it never feeds the engine. (See the matching *Reconciliation note (M4 C4)* at spec 04 §Session-Interrupted Prompt.)
- **iOS:** App kill is detected only at next launch (no AlarmManager watchdog — DESCOPED, spec 10:174). Behaviour is identical — prompt on next launch.
