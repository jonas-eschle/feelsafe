...
| `feedbackPromptRepositoryProvider` | `FeedbackPromptRepository` | `FeedbackPromptRepository()` (SharedPreferences-backed) | inject a fake via `feedbackPromptRepositoryProvider.overrideWithValue(...)` | `wired-real` | spec 04 §Chain Exhausted Screen — Tier-F F5 post-session prompt |
