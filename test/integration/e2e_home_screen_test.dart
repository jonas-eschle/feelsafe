...
List<Override> _base({
  ...
  contactsRepositoryProvider.overrideWithValue(FakeContactsRepository(contacts)),
...
void main() {
  ...
  testWidgets('home_simulate_button_visible_for_default_mode', (tester) async {
    ...
    await tester.pumpWidget(hostScreenWithRouter(
      overrides: _base(modes: [makeMode(id: 'm1', name: 'Walk')]),
      child: const HomeScreen(),
    ));
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'home_start_button_opens_confirmation_dialog',
    (tester) async {
  ...
  testWidgets(
    'home_simulate_button_present_alongside_start',
    (tester) async {
  ...
  testWidgets('home_stealth_enabled_hides_guardian_angela_title', (tester) async {
  ...
  testWidgets('home_stealth_disabled_shows_normal_title', (tester) async {
    ...
    check(find.textContaining('Guardian').evaluate().length).isGreaterOrEqual(1);
  });
