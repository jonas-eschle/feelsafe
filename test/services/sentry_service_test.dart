...
void main() {
  group('RealSentryService', () {
    ...
    test('F10: US DSN (ingest.sentry.io) throws StateError', () async {
      final sdk = _FakeSentrySdk();
      final svc = _real(sdk);
      await expectLater(
        svc.initialize(
    ...
    test('F10: EU DSN (ingest.de.sentry.io) is accepted', () async {
      final sdk = _FakeSentrySdk();
      final svc = _real(sdk);
      await svc.initialize(
        enabled: true,
    ...
    test('F10: StateError message mentions EU host requirement', () async {
      final sdk = _FakeSentrySdk();
      final svc = _real(sdk);
      try {
        await svc.initialize(
    ...
    test('F10: disabled path does not validate DSN (no throw)', () async {
      final sdk = _FakeSentrySdk();
      final svc = _real(sdk);
