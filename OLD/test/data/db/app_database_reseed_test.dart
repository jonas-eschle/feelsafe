...
void main() {
  ...
  test(
    'returns false when the existing DB opens cleanly with the '
    'stored key',
    () async {
  ...
  test(
    'nukes the file and resets the key when the on-disk DB is '
    'corrupted',
    () async {
      final storage = _FakeSecureStorage()..backing[_storageKey] = 'anykey';
      final file = File(p.join(tmp.path, 'guardian_angela.sqlite'));
      await file.writeAsBytes(<int>[
        for (var i = 0; i < 4096; i++) i % 256,
      ]);
      check(file.existsSync()).isTrue();
  ...
  test(
    'an existing encrypted DB can be reopened with the same key '
  ...
  test(
    'nukes the file and resets the key when the stored passphrase '
    'no longer matches the on-disk DB',
    () async {
      final storage = _FakeSecureStorage();
      final file = File(p.join(tmp.path, 'guardian_angela.sqlite'));
