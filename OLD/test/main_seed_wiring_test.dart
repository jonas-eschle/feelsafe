...
void main() {
  test('main.dart-style wiring: seedData populates Walk Mode + Date Mode + '
      'default distress mode on first read of a fresh container', () async {
  ...
  test('second invocation of seedData is idempotent — no duplicates land '
      'in the modes list', () async {
    final db = AppDatabase(executor: NativeDatabase.memory());
    addTearDown(db.close);
    final container = ProviderContainer(
      overrides: [appDatabaseProvider.overrideWithValue(db)],
