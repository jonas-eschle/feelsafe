...
void main() {
  ...
  group('SystemUiService platform branches', () {
    ...
    test(
      'Android: requestBatteryOptimizationExemption invokes native',
      () async {
        final calls = installMethodChannelMock(channel);
        final s = SystemUiService(platform: _android);
        await s.requestBatteryOptimizationExemption();
        check(calls.first.method).equals('requestBatteryOptimizationExemption');
      },
    );

    test(
      'Android: requestBatteryOptimizationExemption surfaces missing',
      () async {
        installMissingPluginMock(channel);
        final s = SystemUiService(platform: _android);
        await check(s.requestBatteryOptimizationExemption()).throws<Object>();
      },
    );

    test(
      'Android: requestBatteryOptimizationExemption rethrows platform err',
      () async {
        installPlatformErrorMock(channel);
        final s = SystemUiService(platform: _android);
        await check(
          s.requestBatteryOptimizationExemption(),
    ...
    test('Android: isBatteryOptimized returns native bool', () async {
      installMethodChannelMock(
        channel,
        responder: (c) => c.method == 'isBatteryOptimized' ? true : null,
      );
      final s = SystemUiService(platform: _android);
    ...
    test(
      'Android: isBatteryOptimized swallows MissingPluginException',
      () async {
        installMissingPluginMock(channel);
        final s = SystemUiService(platform: _android);
        check(await s.isBatteryOptimized()).isFalse();
    ...
    test('Android: isBatteryOptimized rethrows PlatformException', () async {
      installPlatformErrorMock(channel);
      final s = SystemUiService(platform: _android);
  ...
  group('StealthIconService platform branches', () {
    ...
    test('Android: setPreset rethrows PlatformException', () async {
      ...
      final s = StealthIconService(platform: _android);
      await check(
        s.setPreset(StealthIconPreset.weather),
      ).throws<PlatformException>();
    });

    test(
      'Android: getCurrentPreset returns native string parsed to enum',
      () async {
        installMethodChannelMock(
          channel,
    ...
    test('Android: getCurrentPreset unknown string → cached default', () async {
      installMethodChannelMock(
        channel,
        responder: (c) =>
    ...
    test('Android: getCurrentPreset MissingPluginException → cached', () async {
      installMissingPluginMock(channel);
      final s = StealthIconService(platform: _android);
    ...
    test('Android: getCurrentPreset PlatformException → cached', () async {
      ...
      await check(
        s.setPreset(StealthIconPreset.clock),
      ).throws<PlatformException>();
  ...
  group('PhoneService platform branches', () {
    ...
    test(
      'Android: callEmergency invokes native with isEmergency=true',
      () async {
        final calls = installMethodChannelMock(channel);
        final s = PhoneService(platform: _android);
    ...
    test('Android: MissingPluginException falls back to tel: URI '
    ...
    test('Android: PlatformException falls back to tel: URI '
        '(which fails in test harness)', () async {
      installPlatformErrorMock(channel);
      final s = PhoneService(platform: _android);
      await check(s.call('+1234')).throws<Object>();
    ...
    test('iOS: skips native channel, goes straight to tel: URI', () async {
      final calls = installMethodChannelMock(channel);
  ...
  group('MessagingService platform branches', () {
    ...
    test(
      'Android: sendMessage(sms) invokes native send, emits queued',
      () async {
    ...
    test(
      'Android: SMS send MissingPluginException surfaces Phase-10 error',
      () async {
    ...
    test(
      'iOS: SMS falls through to url_launcher (tel harness throws)',
      () async {
        final calls = installMethodChannelMock(smsChannel);
        final s = MessagingService(platform: _ios);
        await check(
    ...
    test(
      'Android: retryExhaustedSms after successful send hits native',
      () async {
        final calls = installMethodChannelMock(smsChannel);
        final s = MessagingService(platform: _android);
        final wid = await s.sendMessage(
    ...
    test(
      'Android: retryExhaustedSms MissingPluginException surfaces',
      () async {
    ...
    test('Android: retryExhaustedSms PlatformException is rethrown', () async {
      ...
      installPlatformErrorMock(smsChannel);
      await check(s.retryExhaustedSms(wid.value)).throws<PlatformException>();
      await s.dispose();
    });

    test('Android: native retry_exhausted event fires retry stream', () async {
      ...
      await mock.push({'type': 'retry_exhausted', 'workId': wid.value});
      final ev = await fut;
    ...
    test(
      'Android: native retry_exhausted for unknown workId uses blanks',
      () async {
        const eventChannel = EventChannel('com.guardianangela.app/sms_events');
        final mock = installEventChannelMock(eventChannel);
        installMethodChannelMock(smsChannel);
        final s = MessagingService(platform: _android);
        final fut = s.smsRetryExhausted.first;
        await mock.push({'type': 'retry_exhausted', 'workId': 'never-seen'});
        final ev = await fut;
    ...
    test('Android: generic native event without workId is ignored', () async {
      const eventChannel = EventChannel('com.guardianangela.app/sms_events');
      final mock = installEventChannelMock(eventChannel);
      installMethodChannelMock(smsChannel);
      final s = MessagingService(platform: _android);
    ...
    test(
      'Android: generic native event with status emits delivery update',
      () async {
        const eventChannel = EventChannel('com.guardianangela.app/sms_events');
        final mock = installEventChannelMock(eventChannel);
        installMethodChannelMock(smsChannel);
    ...
    test(
      'Android: event with no type or status defaults to "unknown"',
      () async {
        const eventChannel = EventChannel('com.guardianangela.app/sms_events');
        final mock = installEventChannelMock(eventChannel);
        installMethodChannelMock(smsChannel);
