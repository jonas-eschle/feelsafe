/// Tests for [FakeCallController] — delegates three actions (answer,
/// hangUp, decline) to [SessionController]. With no active session
/// runtime the delegated calls are no-ops but must not throw.
library;
...
void main() {
  group('FakeCallController.build', () {
    test('returns null initial state', () async {
      final container = ProviderContainer(overrides: _overrides());
      addTearDown(container.dispose);
      final v = await container.read(fakeCallControllerProvider.future);
      check(v).isNull();
    });
  });

  group('FakeCallController actions', () {
    test('answer is a no-op when no session is running', () async {
      final container = ProviderContainer(overrides: _overrides());
      addTearDown(container.dispose);
    ...
    test('hangUp is a no-op when no session is running', () async {
      final container = ProviderContainer(overrides: _overrides());
      addTearDown(container.dispose);
      final notifier = container.read(fakeCallControllerProvider.notifier);
      await container.read(fakeCallControllerProvider.future);
    ...
    test('decline is a no-op when no session is running', () async {
      final container = ProviderContainer(overrides: _overrides());
      addTearDown(container.dispose);
      final notifier = container.read(fakeCallControllerProvider.notifier);
      await container.read(fakeCallControllerProvider.future);
